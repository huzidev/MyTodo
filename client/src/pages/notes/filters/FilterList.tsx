...
export default function FilterList(props: any): JSX.Element {
  ...
  const allItems = [
    ...new Set(allNotes.map((currentEle: any) => currentEle.category)),
  ];
  ...
  return (
    <div>
      {
        <>
          {(completedNotesPath && (status !== true || error)) ||
          (addNotesPath && (status !== false || error)) ? (
            ...
            <div style={style}>
              ...
              <div style={style}>
                <Button type="ghost" className="Dark" onClick={handleClick}>
                  Filter the list
                </Button>
                <Menu
                  id="fade-menu"
                ...
                >
                  <MenuItem onClick={() => setItems(allNotes)}>
                    All items
                  </MenuItem>
      ...
      <Row gutter={16}>
        ...
        {Data.map((note: any) => {
          return (!note.isCompleted && addNotesPath) ||
            (note.isCompleted && completedNotesPath) ? (
